...
| 12 | bytes_per_elem | KV/Index dtype 字节数（BF16=2；INT8/C8/fp8=1）；**自动判定**见下方「KV cache dtype 自动判定」 | `CacheConfig.cache_dtype` → `get_dtype_size`；`vllm_ascend/quantization/modelslim_config.py` |
...
## I. KV cache dtype 自动判定（bytes_per_elem，无需问用户）

行 12 的 `bytes_per_elem` 由 `fill_table.py::detect_bpe` 按优先级自动决定，对齐 vLLM / vLLM-Ascend 源码：

| 优先级 | 判定依据 | 结果 | 源码 |
|--------|---------|------|------|
| 1（最高） | 拉起命令 `--kv-cache-dtype`：`fp8`/`fp8_e4m3`/`fp8_e5m2`/`int8` | 1 字节 | `vllm/config/cache.py: CacheConfig.cache_dtype`（默认 `"auto"`，可选 `fp8`/`fp8_e4m3`/`fp8_e5m2`/`int8` 等） |
| 1 | 拉起命令 `--kv-cache-dtype`：`auto`/`bfloat16`/`float16` | 2 字节 | 同上 |
| 2 | checkpoint 的 `quant_model_description.json`：`kv_cache_type == "C8"` | 1 字节（INT8 KV） | `vllm_ascend/quantization/modelslim_config.py`（`kv_cache_type=="C8"` 时 `kv_cache_torch_dtype = torch.int8`） |
| 2 | 同上文件 `kv_cache_type` 非 C8 / 为空 | 2 字节（BF16） | 同上 |
| 3（兜底） | 都没有 | 2 字节（vLLM `cache_dtype="auto"` → 模型 dtype） | `vllm/config/cache.py` |

> ⚠️ **W8A8 ≠ C8**：`--quantization ascend` 仅量化权重（W8A8），KV cache 仍为 BF16（2 字节）；
> 只有 **W8A8C8** 模型（checkpoint 含 `quant_model_description.json` 且 `kv_cache_type="C8"`）才是 INT8 KV cache（1 字节）。
> C8 由 checkpoint 元数据驱动，**无需 CLI flag**，vLLM-Ascend 检测到即自动启用。

`quant_model_description.json` 同样从 HF / ModelScope 自动下载（仅几 KB，与 config.json 同源），本地模型则读取 config.json 同目录下的同名文件。
